...
exports.toWeierstrass = toWeierstrass;
exports.toTwistedEdwards = toTwistedEdwards;
exports.pointToWeirstrass = pointToWeirstrass;
exports.prepareGaragaHints = prepareGaragaHints;
 ...
 */
function toWeierstrass(a_twisted, d_twisted, p, x_twisted, y_twisted) {
    const a = a_twisted;
    const d = d_twisted;
    const x = (0, ring_sig_utils_1.mod)((5n * a + a * y_twisted - 5n * d * y_twisted - d) *
        (0, ring_sig_utils_1.modPow)(12n - 12n * y_twisted, -1n, p), p);
    const y = (0, ring_sig_utils_1.mod)((a + a * y_twisted - d * y_twisted - d) *
        (0, ring_sig_utils_1.modPow)(4n * x_twisted - 4n * x_twisted * y_twisted, -1n, p), p);
    return [x, y];
}
/*
 ...
 * @returns A tuple [x, y] representing the point in Weierstrass coordinates
 */
function toTwistedEdwards(a_twisted, d_twisted, p, x_weierstrass, y_weierstrass) {
    const a = a_twisted;
    const d = d_twisted;
    const y = (0, ring_sig_utils_1.mod)((5n * a - 12n * x_weierstrass - d) *
        (0, ring_sig_utils_1.modPow)(-12n * x_weierstrass - a + 5n * d, -1n, p), p);
    const x = (0, ring_sig_utils_1.mod)((a + a * y - d * y - d) *
        (0, ring_sig_utils_1.modPow)(4n * y_weierstrass - 4n * y_weierstrass * y, -1n, p), p);
    return [x, y];
}
/**
 * Converts a point from Twisted Edwards to Weierstrass coordinates.
 ...
 */
function pointToWeirstrass(p) {
    switch (p.curve.name) {
        case ring_sig_utils_1.CurveName.ED25519: {
            return toWeierstrass(ED25519_CONSTANTS.A, ED25519_CONSTANTS.D, ED25519_CONSTANTS.P, p.x, p.y);
        }
        default:
 ...
 */
function getPointCoordinates(points, curveName) {
    switch (curveName) {
        case ring_sig_utils_1.CurveName.ED25519:
            return points.map((point) => {
                const [x, y] = toWeierstrass(ED25519_CONSTANTS.A, ED25519_CONSTANTS.D, ED25519_CONSTANTS.P, point.x, point.y);
                return [x, y];
            });
        case ring_sig_utils_1.CurveName.SECP256K1:
...
function getCurveId(curveName, garaga) {
    switch (curveName) {
        case ring_sig_utils_1.CurveName.ED25519:
 ...
 */
async function prepareGaragaHints(points, scalars) {
