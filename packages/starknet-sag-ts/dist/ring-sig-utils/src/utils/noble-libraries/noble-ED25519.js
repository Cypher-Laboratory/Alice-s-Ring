...
class Point {
    ...
    toAffine() {
        ...
        if (this.equals(I))
            return { x: 0n, y: 1n }; // affine zero point (0 , 1)
...
const modL_LE = (hash) => (0, exports.mod)(b2n_LE(hash), N); // mod L; but little-endian
let _shaS;
...
const _sign = (e, rBytes, msg) => {
    ...
    const r = modL_LE(rBytes); // r was created outside, reduce it mod L
...
const _verify = (sig, msg, pub, opts = dvo) => {
    ...
    const finish = (hashed) => {
        ...
        const k = modL_LE(hashed); // decode in little-endian, mod L
