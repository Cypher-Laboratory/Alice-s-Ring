...
type BC = {
  seed: Bytes;
  k2sig: (kb: Bytes) => SignatureWithRecovery | undefined;
}; // Bytes+predicate checker
function prepSig(msgh: Hex, priv: PrivKey, opts = optS): BC {
  ...
  const k2sig = (kBytes: Bytes): SignatureWithRecovery | undefined => {
...
function hmacDrbg<T>(
  asynchronous: true,
): (seed: Bytes, predicate: Pred<T>) => Promise<T>;
function hmacDrbg<T>(
  asynchronous: false,
): (seed: Bytes, predicate: Pred<T>) => T;
...
function hashToPrivateKey(hash: Hex): Bytes {
  ...
  hash = toU8(hash); // produces private keys with mod bias
