  ...
  static deserialize(compressed: string): Point {
    ...
    switch (curve.name) {
      case CurveName.SECP256K1: {
        ...
        const y = modPow(
          mod(modPow(x, 3n, curve.P) + 7n, curve.P),
          (curve.P + 1n) / 4n,
          curve.P,
        );
      ...
      case CurveName.ED25519: {
        ...
        const y2 = mod(y * y, curve.P); // y²
        const u = mod(y2 - 1n, curve.P); // u=y²-1
        const v = mod(d * y2 + 1n, curve.P); // v=dy²+1
