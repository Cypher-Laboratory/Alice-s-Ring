...
describe("Test Constructor", () => {
    ...
    describe("Test constructor with invalid parameters", () => {
        ...
        it("Should throw if msg is empty - ed25519", () => {
            expect(() => new src_1.RingSignature("", data.publicKeys_ed25519, data.randomC, data.randomResponses, ed25519)).toThrow(errors_1.noEmptyMsg);
        });
        it("Should throw if msg is empty - secp256k1", () => {
            expect(() => new src_1.RingSignature("", data.publicKeys_secp256k1, data.randomC, data.randomResponses, secp256k1)).toThrow(errors_1.noEmptyMsg);
        });
        /* -------------TEST INVALID RING------------- */
        ...
        it("Should throw if one point is (0, 0) - ed25519", () => {
            const ring = data.publicKeys_ed25519.slice(1);
            expect(() => new src_1.RingSignature(data.message, [new src_1.Point(ed25519, [0n, 0n])].concat(ring), data.randomC, data.randomResponses, ed25519)).toThrow((0, errors_1.invalidParams)("Point is not on curve: 0,0"));
        });
        it("Should throw if one point is (0, 0) - secp256k1", () => {
            const ring = data.publicKeys_secp256k1.slice(1);
            expect(() => new src_1.RingSignature(data.message, [new src_1.Point(secp256k1, [0n, 0n])].concat(ring), data.randomC, data.randomResponses, secp256k1)).toThrow((0, errors_1.invalidParams)("Point is not on curve: 0,0"));
        });
        ...
        it("Should throw if c is 0 - ed25519", () => {
            expect(() => new src_1.RingSignature(data.message, data.publicKeys_ed25519, 0n, data.randomResponses, ed25519)).toThrow((0, errors_1.invalidParams)("c"));
        });
        it("Should throw if c is 0 - secp256k1", () => {
            expect(() => new src_1.RingSignature(data.message, data.publicKeys_secp256k1, 0n, data.randomResponses, secp256k1)).toThrow((0, errors_1.invalidParams)("c"));
        });
        /* -------------TEST UNKNOWN CURVE------------- */
        ...
        it("Should pass if config.evmCompatibility is true", () => {
            expect(() => new src_1.RingSignature(data.message, data.publicKeys_ed25519, data.randomC, data.randomResponses, ed25519, { evmCompatibility: true }));
        });
        it("Should pass if config.evmCompatibility is false", () => {
            expect(() => new src_1.RingSignature(data.message, data.publicKeys_ed25519, data.randomC, data.randomResponses, ed25519, { evmCompatibility: false }));
        });
        ...
        it("Should pass if config.safeMode is true", () => {
            expect(() => new src_1.RingSignature(data.message, data.publicKeys_ed25519, data.randomC, data.randomResponses, ed25519, { safeMode: true }));
        });
        it("Should pass if config.safeMode is false", () => {
            expect(() => new src_1.RingSignature(data.message, data.publicKeys_ed25519, data.randomC, data.randomResponses, ed25519, { safeMode: false }));
        });
