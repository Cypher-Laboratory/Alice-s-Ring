...
import {ringSignature} from "./ring_signator";
import{sigVerif} from "./ring_verifier";
const privKey =
  randomBigint(
...
const sig = ringSignature("1000", ring, privKey);
...
console.log(
  "verify: ",
  sigVerif("1000",sig?.cvalue ?? "sksksks",sig?.responses ?? [], sig?.ring ?? [[0n,0n]]),
);
